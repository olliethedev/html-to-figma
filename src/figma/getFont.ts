...
export const defaultFont = (style: string = 'Regular') => ({
    family: 'Roboto',
    style,
});
...
export async function getMatchingFont(
    fontStr: string,
    ...
    for (const family of familySplit) {
        ...
        for (const availableFont of availableFonts) {
            ...
            if (
                ...
                availableFont.fontName.style === fontStyle
            ) {
                ...
                fontCache[`${normalizedAvailable}-${fontStyle}`] =
